...
p = `r pv(fisher.test(with(dados, table(CAM, SEXO))))`
...
```{r freq-sex-grupo, fig.cap="**Figura xx** Frequências dos tipos de impacto"}
bar.imp.sex.grupo
```
...
A correlação negativa tem efeito menos marcante no grupo de quadris dolorosos que no grupo controle (figura xx).
...
Quando controlamos pelo tipo de impacto, não é mais possível identificar associação entre os ângulos (figura xx).
