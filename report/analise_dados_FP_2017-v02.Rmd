...
Em nossa amostra, o CAM é `r fisher.OR(ft.sexo.cam, 1)` vezes mais frequente em homens que em mulheres (`r format.OR(ft.sexo.cam)`).

Nossa amostra também indica que impacto do tipo Pincer é `r format.pct(-as.numeric(fisher.OR(ft.sexo.pincer, 2)), 0, pct.symbol = TRUE)`  menos frequente em homens que em mulheres (`r format.OR(ft.sexo.pincer)`).
Isso confirma que a maior prevalência de Pincer é em mulheres.

Não há evidências em nossa amostra de que um impacto do tipo Misto acometa mais homens que mulheres (`r format.OR(ft.sexo.misto)`).
...
Observamos que a presença de CAM é `r fisher.OR(ft.grupo.cam, 1)` vezes mais frequente nos quadris dolorosos (`r format.OR(ft.grupo.cam)`) que no grupo controle.
Não houve diferença significativa nas ocorrências para Pincer (`r format.OR(ft.grupo.pincer)`) e misto (`r format.OR(ft.grupo.misto)`) (Figura 33).
...
O ângulo alfa sozinho, no entanto, explica apenas $r^2=$ `r format.pct(corr.tor.alf$estimate^2, pct.symbol = TRUE)` da variabilidade observada no ângulo de torção femoral.
